print("CCU002-04 R code adapted from Hoda and Samantha IP for project comparing CVD outcomes in influenza/pneumonia versus COVID-19")
print("Spencer Keene")

#remove all objects
rm(list=ls())

#df <- data.table::fread("P:/keenes/Projects/CCU002-04/raw/CCU002_04_COVID_COHORT_20220329.csv",
#                        data.table = FALSE)
...
week1<-"_" # place "week1" for 4 time periods and  "_" otherwise

censor<-"nocensor" #censor OR nocensor for non-hospitalised infection


#COVID dates are 2020-01-01 to 2021-12-31
...
cohort_start_date_all <- as.Date("2020-01-01")
cohort_end_date_all <- as.Date("2021-12-31")

  df$cohort_start_date <- as.Date("2020-01-01")
  df$cohort_end_date <- as.Date("2021-12-31")


clist <- c("hospitalised") #"non-hospitalised"
for(i in clist) {
  pheno_of_interest <- i
  print(pheno_of_interest)

#pheno_of_interest<-"hospitalised"
#pheno_of_interest<-"non-hospitalised"
...
covariates <- df1[, grep(pattern="COV_|COM_|COINF|hx_|CARE_|ICU_|RURAL_U", colnames(df1))]
...
df1 <- df1 %>% select(-starts_with(c("COV_","COM_","COINF","hx_","CARE_","ICU_","RURAL_U"))) 
...
#changed below from cohort_end_date and cohort_start_date to cohort_end_date_all and cohort_start_date_all 
...
outcomes_names2 <- outcomes_names2[outcomes_names2 != "OUT_ARTERIAL_EVENT.1"]
...
if (censor!="censor") {
  data$DATE_EXPO_CENSOR <- as.Date(ifelse(!(data$EXP_CONFIRMED_COVID_PHENOTYPE %in% pheno_of_interest),
                                          data$cohort_end_date,
                                          NA), origin='1970-01-01') 
  
  data$expo_date <- as.Date(ifelse((!is.na(data$DATE_EXPO_CENSOR)) &
                                     (data$cohort_end_date >= data$DATE_EXPO_CENSOR), #do i need >= here? or below?
                                   NA, data$EXP_CONFIRMED_COVID19_DATE), origin='1970-01-01') #phenotype: code added
  
} else {

  data$DATE_EXPO_CENSOR <- as.Date(ifelse(!(data$EXP_CONFIRMED_COVID_PHENOTYPE %in% pheno_of_interest),
                                          data$EXP_CONFIRMED_COVID19_DATE,
                                          NA), origin='1970-01-01') 
  
  data$expo_date <- as.Date(ifelse((!is.na(data$DATE_EXPO_CENSOR)) &
                                     (data$EXP_CONFIRMED_COVID19_DATE >= data$DATE_EXPO_CENSOR),
                                   NA, data$EXP_CONFIRMED_COVID19_DATE), origin='1970-01-01') #phenotype: code added
...
#  data$covid_inf<-ifelse(data$EXP_CONFIRMED_COVID19_DATE>=cohort_start_date & data$EXP_CONFIRMED_COVID19_DATE<=data$pt_end_date,1,0)
  #change the below from cohort_start_date to data$cohort_start_date
  data$covid_inf<-ifelse((!is.na(data$expo_date) & (data$expo_date>=data$cohort_start_date & data$expo_date<=data$pt_end_date)),1,0) #phenotype: code added
...
#  data$covid_inf[is.na(data$EXP_CONFIRMED_COVID19_DATE)]=0

#  data$EXP_CONFIRMED_COVID_PHENOTYPE<-ifelse(data$covid_inf==0,data$EXP_CONFIRMED_COVID_PHENOTYPE=="",data$EXP_CONFIRMED_COVID_PHENOTYPE)
...
#  data$fup<-data$fup+1
  ...
  data_sample_uninfected_noncases <- 
    data[sample(which(data$overall_event==0 & data$covid_inf==0), round(noncase_frac*length(which(data$overall_event==0 & data$covid_inf==0)))), ]
  data_infected_noncases <- data[which(data$overall_event==0 & data$covid_inf==1),]
  ...
  infected_indiv <- data_sample[which(data_sample$covid_inf==1),]
  #changed the below from cohort_start_date to infected_indiv$cohort_start_date
  infected_indiv$delta_covid_inf<-as.numeric(difftime(infected_indiv$expo_date, infected_indiv$cohort_start_date, unit="days"))
  ...
  vars<-c("ALF_E", "fup", "EXP_CONFIRMED_COVID_PHENOTYPE",
  ...
  td_data <-
    tmerge(
      ...
      data2 = infected_indiv %>% select(all_of(vars), overall_event, delta_covid_inf, covid_inf), #'covid_inf' not needed, 'value' replaced with overall_event, 'fup' added
      id = ALF_E, 
      overall_event = event(fup,  overall_event), 
      covid_inf = tdc(delta_covid_inf)
    )
  ...
  # with_expo$EXP_CONFIRMED_COVID_PHENOTYPE[with_expo$EXP_CONFIRMED_COVID_PHENOTYPE =="" | with_expo$EXP_CONFIRMED_COVID_PHENOTYPE ==" "] <- NA
  # with_expo <- mutate_at(with_expo, c("EXP_CONFIRMED_COVID_PHENOTYPE"), ~replace(., is.na(.), 0))
  # 
  # with_expo$hospitalised <- ifelse(with_expo$EXP_CONFIRMED_COVID_PHENOTYPE=="hospitalised",1, 0)
  # with_expo$hospitalised<-as.numeric(with_expo$hospitalised)
  # with_expo$nonhospitalised <- ifelse(with_expo$EXP_CONFIRMED_COVID_PHENOTYPE=="hospitalised",0, 1)
  # with_expo$nonhospitalised<-as.numeric(with_expo$nonhospitalised)
  # 
  # with_expo$hosp_label <- ifelse(with_expo$hospitalised==1, "Hospitalised_COVID", "Non-hospitalised_COVID")
  # with_expo$hosp_label <- factor(with_expo$hosp_label, levels = c("Non-hospitalised_COVID", "Hospitalised_COVID"))
  # with_expo$hosp_label = relevel(with_expo$hosp_label, ref = "Non-hospitalised_COVID")
  # with_expo$nonhosp_label <- ifelse(with_expo$nonhospitalised==1, "Non-hospitalised_COVID", "Hospitalised_COVID")
  # with_expo$nonhosp_label <- factor(with_expo$nonhosp_label, levels = c("Hospitalised_COVID", "Non-hospitalised_COVID"))
  # with_expo$nonhosp_label = relevel(with_expo$nonhosp_label, ref = "Hospitalised_COVID")
  
  
  ###########  prepare dataframe for those  without infection    ##################
  without_expo <- data_sample[which(data_sample$covid_inf==0),]
  ...
  # without_expo$hosp_label <- ifelse(without_expo$hospitalised==1, "Hospitalised_COVID", "Non-hospitalised_COVID")
  # without_expo$hosp_label <- factor(without_expo$hosp_label, levels = c("Non-hospitalised_COVID", "Hospitalised_COVID"))
  # without_expo$hosp_label = relevel(without_expo$hosp_label, ref = "Non-hospitalised_COVID")
  # without_expo$nonhosp_label <- ifelse(without_expo$nonhospitalised==1, "Non-hospitalised_COVID", "Hospitalised_COVID")
  # without_expo$nonhosp_label <- factor(without_expo$nonhosp_label, levels = c("Hospitalised_COVID", "Non-hospitalised_COVID"))
  # without_expo$nonhosp_label = relevel(without_expo$nonhosp_label, ref = "Hospitalised_COVID")
                                               ...
                                               ifelse(week_cols$week2_4==1,"week2_4",
                                                      ifelse(week_cols$week5_16==1,"week5_16",
                                                             ifelse(week_cols$week17_75==1, "week17_75",
                                                                    ifelse(week_cols$week76_105==1, "week76_105",NA))))))
  ...
  #
  write.csv(tbl_event_count, paste0("P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/event_counts/tbl_event_count_COVID_",outcome_list2,"_",time,"_",vaccine,"_",pheno_of_interest,"_",censor,"_",excl,"_",week1,"_",Sys.Date(),".csv"), row.names = T)

  #=============================================================================
  # ============================= INCIDENCE RATE =================================
  #=============================================================================
  data_surv$fup2<-data_surv$tstop - data_surv$tstart
  infected<-data_surv[which(data_surv$covid_inf==1),]
  controls<-data_surv[which(data_surv$covid_inf==0),]
  ...
  incrate <- data.table::fread(paste0("P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/event_counts/tbl_event_count_COVID_",outcome_list2,"_",time,"_",vaccine,"_",pheno_of_interest,"_",censor,"_",excl,"_",week1,"_",Sys.Date(),".csv"),
                               data.table = FALSE)
  ...
  write.table(inc_rate3, paste0("P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/incidence/tbl_incidence_rate_COVID_overall_",time,"_",vaccine,"_",pheno_of_interest,"_",censor,"_",excl,"_",week1,"_",Sys.Date(),".csv"), append=TRUE, col.name=!file.exists(paste0("P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/incidence/tbl_incidence_rate_COVID_overall_",time,"_",vaccine,"_",pheno_of_interest,"_",censor,"_",excl,"_",week1,"_",Sys.Date(),".csv")))
  write.table(inc_rate4, paste0("P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/incidence/tbl_incidence_rate_COVID_infected_",time,"_",vaccine,"_",pheno_of_interest,"_",censor,"_",excl,"_",week1,"_",Sys.Date(),".csv"), append=TRUE, col.name=!file.exists(paste0("P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/incidence/tbl_incidence_rate_COVID_infected_",time,"_",vaccine,"_",pheno_of_interest,"_",censor,"_",excl,"_",week1,"_",Sys.Date(),".csv")))
  write.table(inc_rate5, paste0("P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/incidence/tbl_incidence_rate_COVID_controls_",time,"_",vaccine,"_",pheno_of_interest,"_",censor,"_",excl,"_",week1,"_",Sys.Date(),".csv"), append=TRUE, col.name=!file.exists(paste0("P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/incidence/tbl_incidence_rate_COVID_controls_",time,"_",vaccine,"_",pheno_of_interest,"_",censor,"_",excl,"_",week1,"_",Sys.Date(),".csv")))
  ...
  if (week1=="week1") {
  cov_model <- "week1 + week2_4 + week5_16 + week17_75 + COV_AGE + COV_SEX + age_sq + COV_DEPRIVATION + COV_EVER_DEMENTIA + COV_EVER_CKD + hx_hypertension + COV_SMOKING_STATUS + COV_ELIXHAUSER_SCORE + COV_EVER_OBESITY + COV_EVER_COPD + COV_EVER_CANCER + COV_EVER_DIABETES + COV_EVER_ANY_CVD + COV_SURGERY_LASTYR + COV_LIPID_MEDS + hx_antiplat_anticoag + CARE_HOME + RURAL_URBAN + COV_EVER_DEPRESSION + COV_EVER_FRACTURE"
  cov <-c("week1 ", "week2_4 ", "week5_16 ", "week17_75 ", "age ", "sex ", "age_squared ", "DEPRIVATIONMissing", "DEPRIVATION2", "DEPRIVATION3", "DEPRIVATION4", "DEPRIVATION5", "EVER_DEMENTIA", "EVER_CKD"
          , "hypertension ", "SMOKING_STATUS_Missing ", "SMOKING_STATUS_Ex-smoker ", "SMOKING_STATUS_Current-smoker ",
          "ELIXHAUSER_SCORE ", "EVER_OBESITY ", "EVER_COPD ", "EVER_CANCER ", "EVER_DIABETES ", 
          "EVER_ANY_CVD ", "SURGERY_LASTYR ", "LIPID_MEDS ", "antiplat_anticoag ", "CARE_HOME ",
          "RURAL_URBAN ", "COV_EVER_DEPRESSION ", "COV_EVER_FRACTURE ")
  ...
  write(c("event term estimate conf.low conf.high std.error n.event subgroup"), 
        paste0('P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/',time,'/',adjust,'_cox_results_COVID_',week1,"_",Sys.Date(),"_",vaccine,"_",pheno_of_interest,"_",outcome_list2,"_",censor,"_",excl,'.csv'), append = TRUE)
  ...
  purrr::map(outcome_list, function(x) {
    ...
    cat(paste0(substring(x,9,40),' ',cov, apply(s$coefficients, 1, 
               ...
               collapse = '\n'), '\n', sep = '', 
        file = paste0('P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/',time,'/',adjust,'_cox_results_COVID_',week1,"_",Sys.Date(),"_",vaccine,"_",pheno_of_interest,"_",outcome_list2,"_",censor,"_",excl,'.csv'), append = TRUE)
    invisible(model)
  })
  
  #Read in "Extensive..." results files after running Coxph then do the following
  estimates <- data.table::fread(paste0('P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/',time,'/',adjust,'_cox_results_COVID_',week1,"_",Sys.Date(),"_",vaccine,"_",pheno_of_interest,"_",outcome_list2,"_",censor,"_",excl,'.csv'),
                                 data.table = FALSE)
  ...
  if (week1=="week1") {
    tmp2<- data.frame(term=c("week1", "week2_4", "week5_16", "week17_75"),
                      Weeks_category=c(1,2,3,4))
  } 
  ...
  if (week1!="week1") {
    ...
    estimates$add_to_median <- ifelse(estimates$term=="week1", 0, estimates$add_to_median)
  }
  ...
  #Then save to csv.
  write.table(estimates, paste0('P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/',time,'/',adjust,'_cox_results_COVID_',week1,"_",Sys.Date(),"_",vaccine,"_",pheno_of_interest,"_",outcome_list2,"_",censor,"_",excl,'.csv'), row.names = FALSE)
  
  
  
if (outcome_list2=="AMI" | outcome_list2=="DVT_DVT" | outcome_list2=="FRACTURE" | 
    outcome_list2=="PE" | outcome_list2=="STROKE_ISCH" | outcome_list2=="STROKE_SAH_HS" | 
    outcome_list2=="ARTERIAL_EVENT" | outcome_list2=="VENOUS_EVENT") {
    
  

  # For stratification by Age group
  #-------------------------------------------------------------------------------
  
  write(c("event term estimate conf.low conf.high std.error n.event subgroup"), 
        paste0('P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/',time,'/',adjust,'_agegroup_cox_results_COVID_',week1,"_",Sys.Date(),"_",vaccine,"_",pheno_of_interest,"_",outcome_list2,"_",censor,"_",excl,'.csv'), append = TRUE)
  
  lapply(split(survival_data, survival_data$agegroup), function(y) 
    purrr::map(outcome_list, function(x) {
      f <- as.formula(paste("Surv(time = tstart, time2 = tstop, event = ", x, ") ~ ",cov_model))
      model <- coxph(f, y, id = ALF_E, weights = y$cox_weights, cluster = ALF_E, robust = T)
      model$call$formula <- f 
      s <- summary(model)
      cat(paste0(substring(x,9,40),' ',cov, apply(s$coefficients, 1, 
                                                  function(x) {
                 ...
                 " ", y$agegroup[1],
                 collapse = '\n'), '\n', sep = '', 
          file = paste0('P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/',time,'/',adjust,'_agegroup_cox_results_COVID_',week1,"_",Sys.Date(),"_",vaccine,"_",pheno_of_interest,"_",outcome_list2,"_",censor,"_",excl,'.csv'), append = TRUE)
      invisible(model)
    })
  )
  
  #Read in "Extensive..." results files after running Coxph then do the following
  estimates <- data.table::fread(paste0('P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/',time,'/',adjust,'_agegroup_cox_results_COVID_',week1,"_",Sys.Date(),"_",vaccine,"_",pheno_of_interest,"_",outcome_list2,"_",censor,"_",excl,'.csv'),
                                 data.table = FALSE)
  estimates <- estimates %>% filter(term %in% term[grepl("^week",term)])
  tmp2<- data.frame(term=c("week1_0.142857143", "week1.142857143_1", "week2_4", "week5_16", "week17_75"),
                    Weeks_category=c(1,2,3,4,5))
  ...
  #Then save to csv.
  write.table(estimates, paste0('P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/',time,'/',adjust,'_agegroup_cox_results_COVID_',week1,"_",Sys.Date(),"_",vaccine,"_",pheno_of_interest,"_",outcome_list2,"_",censor,"_",excl,'.csv'), row.names = FALSE)
  

  # For stratification by sex
  #-------------------------------------------------------------------------------

  write(c("event term estimate conf.low conf.high std.error n.event subgroup"),
        paste0('P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/',time,'/',adjust,'_Sex_cox_results_COVID_',week1,"_",Sys.Date(),"_",vaccine,"_",pheno_of_interest,"_",outcome_list2,"_",censor,"_",excl,'.csv'), append = TRUE)

  lapply(split(survival_data, survival_data$sex_label), function(y) #sex_label has replace COV_SEX here
    purrr::map(outcome_list, function(x) {
      f <- as.formula(paste("Surv(time = tstart, time2 = tstop, event = ", x, ") ~ ",cov_model))
      model <- coxph(f, y, id = ALF_E, weights = y$cox_weights, cluster = ALF_E, robust = T)
      model$call$formula <- f
      ...
      cat(paste0(substring(x,9,40),' ',cov, apply(s$coefficients, 1,
                 ...
                 " ", y$sex_label[1],
                 collapse = '\n'), '\n', sep = '',
          file = paste0('P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/',time,'/',adjust,'_Sex_cox_results_COVID_',week1,"_",Sys.Date(),"_",vaccine,"_",pheno_of_interest,"_",outcome_list2,"_",censor,"_",excl,'.csv'), append = TRUE)
      invisible(model)
    })
  )

  #Read in "Extensive..." results files after running Coxph then do the following
  estimates <- data.table::fread(paste0('P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/',time,'/',adjust,'_Sex_cox_results_COVID_',week1,"_",Sys.Date(),"_",vaccine,"_",pheno_of_interest,"_",outcome_list2,"_",censor,"_",excl,'.csv'),
                                 data.table = FALSE)
  estimates <- estimates %>% filter(term %in% term[grepl("^week",term)])
  tmp2<- data.frame(term=c("week1_0.142857143", "week1.142857143_1", "week2_4", "week5_16", "week17_75"),
                    Weeks_category=c(1,2,3,4,5))
  ...
  #Then save to csv.
  write.table(estimates, paste0('P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/',time,'/',adjust,'_Sex_cox_results_COVID_',week1,"_",Sys.Date(),"_",vaccine,"_",pheno_of_interest,"_",outcome_list2,"_",censor,"_",excl,'.csv'), row.names = FALSE)




  # For stratification by history of CVD
  #-------------------------------------------------------------------------------

  write(c("event term estimate conf.low conf.high std.error n.event subgroup"),
        paste0('P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/',time,'/',adjust,'_hxCVD_cox_results_COVID_',week1,"_",Sys.Date(),"_",vaccine,"_",pheno_of_interest,"_",outcome_list2,"_",censor,"_",excl,'.csv'), append = TRUE)

  lapply(split(survival_data, survival_data$cvd_label), function(y)
    purrr::map(outcome_list, function(x) {
      f <- as.formula(paste("Surv(time = tstart, time2 = tstop, event = ", x, ") ~ ",cov_model))
      model <- coxph(f, y, id = ALF_E, weights = y$cox_weights, cluster = ALF_E, robust = T)
      model$call$formula <- f
      ...
      cat(paste0(substring(x,9,40),' ',cov, apply(s$coefficients, 1,
                 ...
                 " ", y$cvd_label[1],
                 collapse = '\n'), '\n', sep = '',
          file = paste0('P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/',time,'/',adjust,'_hxCVD_cox_results_COVID_',week1,"_",Sys.Date(),"_",vaccine,"_",pheno_of_interest,"_",outcome_list2,"_",censor,"_",excl,'.csv'), append = TRUE)
      invisible(model)
    })
  )

  #Read in "Extensive..." results files after running Coxph then do the following
  estimates <- data.table::fread(paste0('P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/',time,'/',adjust,'_hxCVD_cox_results_COVID_',week1,"_",Sys.Date(),"_",vaccine,"_",pheno_of_interest,"_",outcome_list2,"_",censor,"_",excl,'.csv'),
                                 data.table = FALSE)
  estimates <- estimates %>% filter(term %in% term[grepl("^week",term)])
  tmp2<- data.frame(term=c("week1_0.142857143", "week1.142857143_1", "week2_4", "week5_16", "week17_75"),
                    Weeks_category=c(1,2,3,4,5))
  ...
  #Then save to csv.
  write.table(estimates, paste0('P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/',time,'/',adjust,'_hxCVD_cox_results_COVID_',week1,"_",Sys.Date(),"_",vaccine,"_",pheno_of_interest,"_",outcome_list2,"_",censor,"_",excl,'.csv'), row.names = FALSE)


  # For stratification by Antiplatelet medication
  #-------------------------------------------------------------------------------

  write(c("event term estimate conf.low conf.high std.error n.event subgroup"),
        paste0('P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/',time,'/',adjust,'_antiplatelet_cox_results_COVID_',week1,"_",Sys.Date(),"_",vaccine,"_",pheno_of_interest,"_",outcome_list2,"_",censor,"_",excl,'.csv'), append = TRUE)

  lapply(split(survival_data, survival_data$antiplatelet_label), function(y)
    purrr::map(outcome_list, function(x) {
      f <- as.formula(paste("Surv(time = tstart, time2 = tstop, event = ", x, ") ~ ",cov_model))
      model <- coxph(f, y, id = ALF_E, weights = y$cox_weights, cluster = ALF_E, robust = T)
      model$call$formula <- f
      ...
      cat(paste0(substring(x,9,40),' ',cov, apply(s$coefficients, 1,
                                                  function(x) {
                                                    paste0(" ", round(exp(x[1]), 3), #change to cov1 and cov2 depending on the list of covariates used
                                                           ...
                                                           " ", round((x[3]), 4),
                                                           " ", summary(model)$nevent)}),
                 " ", y$antiplatelet_label[1],
                 collapse = '\n'), '\n', sep = '',
          file = paste0('P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/',time,'/',adjust,'_antiplatelet_cox_results_COVID_',week1,"_",Sys.Date(),"_",vaccine,"_",pheno_of_interest,"_",outcome_list2,"_",censor,"_",excl,'.csv'), append = TRUE)
      invisible(model)
    })
  )

  #Read in "Extensive..." results files after running Coxph then do the following
  estimates <- data.table::fread(paste0('P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/',time,'/',adjust,'_antiplatelet_cox_results_COVID_',week1,"_",Sys.Date(),"_",vaccine,"_",pheno_of_interest,"_",outcome_list2,"_",censor,"_",excl,'.csv'),
                                 data.table = FALSE)
  estimates <- estimates %>% filter(term %in% term[grepl("^week",term)])
  tmp2<- data.frame(term=c("week1_0.142857143", "week1.142857143_1", "week2_4", "week5_16", "week17_75"),
                    Weeks_category=c(1,2,3,4,5))
  ...
  #Then save to csv.
  write.table(estimates, paste0('P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/',time,'/',adjust,'_antiplatelet_cox_results_COVID_',week1,"_",Sys.Date(),"_",vaccine,"_",pheno_of_interest,"_",outcome_list2,"_",censor,"_",excl,'.csv'), row.names = FALSE)
  ...
  cov_model <- "week1_0.142857143:agegroup + week1.142857143_1:agegroup + week2_4:agegroup + week5_16:agegroup + week17_75:agegroup + COV_AGE + COV_SEX + age_sq + COV_DEPRIVATION + COV_EVER_DEMENTIA + COV_EVER_CKD + hx_hypertension + COV_SMOKING_STATUS + COV_ELIXHAUSER_SCORE + COV_EVER_OBESITY + COV_EVER_COPD + COV_EVER_CANCER + COV_EVER_DIABETES + COV_EVER_ANY_CVD + COV_SURGERY_LASTYR + COV_LIPID_MEDS + hx_antiplat_anticoag + CARE_HOME + RURAL_URBAN + COV_EVER_DEPRESSION + COV_EVER_FRACTURE"
  cov <-c("age ", "sex ", "age_squared ", "DEPRIVATIONMissing", "DEPRIVATION2", "DEPRIVATION3", "DEPRIVATION4", "DEPRIVATION5", "EVER_DEMENTIA", "EVER_CKD"
          , "hypertension ", "SMOKING_STATUS_Missing ", "SMOKING_STATUS_Ex-smoker ", "SMOKING_STATUS_Current-smoker ",
          "ELIXHAUSER_SCORE ", "EVER_OBESITY ", "EVER_COPD ", "EVER_CANCER ", "EVER_DIABETES ", 
          "EVER_ANY_CVD ", "SURGERY_LASTYR ", "LIPID_MEDS ", "antiplat_anticoag ", "CARE_HOME ",
          "RURAL_URBAN ", "COV_EVER_DEPRESSION ", "COV_EVER_FRACTURE ", 
          "week1_0.142857143:agegroup<60 ", "week1_0.142857143:agegroup60+ ",  
          "week1.142857143_1:agegroup<60 ", "week1.142857143_1:agegroup60+ ",
          "week2_4:agegroup<60 ", "week2_4:agegroup60+ ",
          "week5_16:agegroup<60 ", "week5_16:agegroup60+ ", 
          "week17_75:agegroup<60 ", "week17_75:agegroup60+ ")
  
  write(c("event term estimate conf.low conf.high std.error n.event subgroup"),
        paste0('P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/',time,'/',adjust,'_agegroup2_cox_results_COVID_',week1,"_",Sys.Date(),"_",vaccine,"_",pheno_of_interest,"_",outcome_list2,"_",censor,"_",excl,'.csv'), append = TRUE)


  purrr::map(outcome_list, function(x) {
    f <- as.formula(paste("Surv(time = tstart, time2 = tstop, event=", x, ") ~ ",cov_model))
    model <- coxph(f, survival_data, id = ALF_E, weights = survival_data$cox_weights)
    model$call$formula <- f
    s <- summary(model)
    cat(paste0(substring(x,9,40),' ',cov, apply(s$coefficients, 1,
                                                function(x) {
                                                  paste0(" ", round(exp(x[1]), 3), #change to cov1 and cov2 depending on the list of covariates used
                                                         ...
                                                         " ", summary(model)$nevent)}),
               collapse = '\n'), '\n', sep = '',
        file = paste0('P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/',time,'/',adjust,'_agegroup2_cox_results_COVID_',week1,"_",Sys.Date(),"_",vaccine,"_",pheno_of_interest,"_",outcome_list2,"_",censor,"_",excl,'.csv'), append = TRUE)
    invisible(model)
  })
  }
