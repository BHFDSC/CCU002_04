...
average_daily_incidence<-function(dataset, disease_event_date, covid_date,futime,max_follow_up_time){
  ...
  #change end_date below to two years after start.  add cohort_start as a argument
  #disease_status<-ifelse(!is.na(disease_event_date) & (disease_event_date - cohort_start)>max_follow_up_time,0,1) 
  ...
  exposed_individual<-sum(!is.na(dataset$EXP_CONFIRMED_FLU_PNEUMONIA_DATE))  #number of individuals (exposed) with a COVID-19 diagnosis during cohort follow-up
...
plot_graph<-function(num_subgroups, life_table1,lifetable2,lifetable3,lifetable4,lifetable5,lifetable6,lifetable7, graph_title,graph_name,levels,legend_title,color_value,linetype){
  ...
  if(num_subgroups==0){#if there is no subgroup, i.e. plotting for all and not split by sex for example, put num_subgroups=0 in the function argument.
    ...
    p_line<-ggplot(life_table1,
      ...
      labs(x='Weeks since Flu/Pneumonia hospitalisation',y='Difference in cumulative absolute risk (%)',
           title = graph_title)+
  ...
  p_line<-ggplot(combined,
    ...
    #    scale_y_continuous(breaks = c(0,1,2,3,4),limits = c(0,4))+
    scale_y_continuous(breaks = c(0,1,2,3),limits = c(0,3))+
    labs(x='Weeks since influenza/pneumonia hospitalisation',y='Difference in cumulative absolute risk (%)',
         title = graph_title)+
    theme(plot.title = element_text(hjust = 0.5))+
    #theme_hc()+ this background requires ggthemes package
...
data<-fread("P:/keenes/Projects/CCU002-04/raw/CCU002_04_FLU_COHORT_20230322.csv",
            select = c('ALF_E','DEATH_DATE','GP_COVERAGE_END_DATE', 'LOST_TO_FOLLOWUP_DATE',
                       'COV_SEX','COV_AGE',
                       'EXP_CONFIRMED_FLU_PNEUMONIA_DATE','OUT_VENOUS_EVENT', 'OUT_ARTERIAL_EVENT', 
                       'EXP_CONFIRMED_FLU_PNEUMONIA_PHENOTYPE'))


#2.Define cohort start date and end date
start_date<-as.IDate('2016-01-01')
end_date<-as.IDate('2019-12-31')
...
#5.count the number of individuals that died or have confirmed Flu/Pneumonia diagnosis or have arterial event in the dataset
count_columns(dataset=data, column_names = c('DEATH_DATE','EXP_CONFIRMED_FLU_PNEUMONIA_DATE','OUT_VENOUS_EVENT', 'OUT_ARTERIAL_EVENT'))




#6.calculate the follow-up time that individual spent at risk (no disease event of interest) and without Flu/Pneumonia diagnosis 
#censored when the individual (whichever is earliest):
#died 
#have disease of interest 
#confirmed Flu/Pneumonia diagnosis 
...
#incidence_rate(disease_event_date=data$OUT_VENOUS_EVENT, covid_date=data$EXP_CONFIRMED_FLU_PNEUMONIA_DATE, futime=data$futime)
...
#weeks_since_covid<-c(rep('1',7), rep('2-4',21),rep('5-16',84),rep('17-75',413), rep('76-209',935))
weeks_since_covid<-c(
  rep('1',7),
  rep('2-4',21),rep('5-16',84),rep('17-75',413), rep('76-209',935))


sink("P:/keenes/Projects/CCU002-04/results/FLU_cox_analysis/AER/Flu_excess_events_week1.log",append=FALSE,split=TRUE)

if (censor=="censor") {
  data$CENSOR_DATE <- as.Date(ifelse((data$EXP_CONFIRMED_FLU_PNEUMONIA_PHENOTYPE=="non-hospitalised"),
                                     data$EXP_CONFIRMED_FLU_PNEUMONIA_DATE, NA),origin='1970-01-01')
  
  data$EXP_CONFIRMED_FLU_PNEUMONIA_DATE <- as.Date(ifelse((data$EXP_CONFIRMED_FLU_PNEUMONIA_PHENOTYPE=="non-hospitalised"), 
                                                          NA,data$EXP_CONFIRMED_FLU_PNEUMONIA_DATE),origin='1970-01-01')
...
# data$CENSOR_DATE <- as.IDate(ifelse((data$EXP_CONFIRMED_FLU_PNEUMONIA_PHENOTYPE=="non-hospitalised"),
#                                                 data$EXP_CONFIRMED_FLU_PNEUMONIA_DATE,NA),origin='1970-01-01')
# 
# data$EXP_CONFIRMED_FLU_PNEUMONIA_DATE <- as.Date(ifelse((!is.na(data$CENSOR_DATE)) & 
#                                    (data$EXP_CONFIRMED_FLU_PNEUMONIA_DATE >= data$CENSOR_DATE), 
#                                  NA, data$EXP_CONFIRMED_FLU_PNEUMONIA_DATE), origin='1970-01-01') #phenotype: code added


#max_follow_up_time<-511 #place this here temporarily until I sort the issue. I didn't include all the time since covid covariates in the Cox model.
...
# data$futime<-to_calculate_futime(cohort_start = start_date, covid_date = data$EXP_CONFIRMED_FLU_PNEUMONIA_DATE,death_date = data$DEATH_DATE,
...
data<-transform(data, min_date=pmin(EXP_CONFIRMED_FLU_PNEUMONIA_DATE, DEATH_DATE, LOST_TO_FOLLOWUP_DATE,
...
print("Influenza/pneumonia excess events for Venous outcome")


#Female
print("Female")

return_daily_incidence<-average_daily_incidence(dataset=data_female, disease_event_date=data_female$OUT_VENOUS_EVENT, covid_date=data_female$EXP_CONFIRMED_FLU_PNEUMONIA_DATE, futime=data_female$futime,
                                                max_follow_up_time=max_follow_up_time)

HR<-c(
  rep(62.5,7),
  rep(12.7,21),rep(5.50,84),rep(1.68,413),rep(1.00,935))
HR_LB<-c(
  rep(49.4,7),
  rep(9.21,21),rep(4.16,84),rep(1.28,413),rep(1.00,935)) #HR upper 95% CI
HR_UB<-c(
  rep(79.2,7),
  rep(17.6,21),rep(7.26,84),rep(2.21,413),rep(1.00,935)) #HR lower 95% CI
...
return_daily_incidence<-average_daily_incidence(dataset=data_male, disease_event_date=data_male$OUT_VENOUS_EVENT, covid_date=data_male$EXP_CONFIRMED_FLU_PNEUMONIA_DATE, futime=data_male$futime,
                                                max_follow_up_time=max_follow_up_time)

HR<-c(
  rep(76.2,7),
  rep(10.9,21),rep(6.22,84),rep(2.17,413),rep(1.00,935))
HR_LB<-c(
  rep(60.3,7),
  rep(7.46,21),rep(4.67,84),rep(1.66,413),rep(1.00,935)) #HR upper 95% CI
HR_UB<-c(
  rep(96.2,7),
  rep(16.0,21),rep(8.28,84),rep(2.84,413),rep(1.00,935)) #HR lower 95% CI
...
return_daily_incidence<-average_daily_incidence(dataset=data_young, disease_event_date=data_young$OUT_VENOUS_EVENT, covid_date=data_young$EXP_CONFIRMED_FLU_PNEUMONIA_DATE, futime=data_young$futime,
                        max_follow_up_time=max_follow_up_time)

HR<-c(
  rep(195,7),
  rep(26.6,21),rep(11.5,84),rep(4.52,413),rep(1.00,935))
HR_LB<-c(
  rep(143,7),
  rep(16.0,21),rep(7.58,84),rep(3.22,413),rep(1.00,935)) #HR upper 95% CI
HR_UB<-c(
  rep(268,7),
  rep(44.2,21),rep(17.3,84),rep(6.35,413),rep(1.00,935)) #HR lower 95% CI
...
return_daily_incidence<-average_daily_incidence(dataset=data_old, disease_event_date=data_old$OUT_VENOUS_EVENT, covid_date=data_old$EXP_CONFIRMED_FLU_PNEUMONIA_DATE, futime=data_old$futime,
                        max_follow_up_time=max_follow_up_time)

HR<-c(
  rep(55.9,7),
  rep(10.3,21),rep(5.10,84),rep(1.51,413),rep(1.00,935))
HR_LB<-c(
  rep(46.1,7),
  rep(7.76,21),rep(4.07,84),rep(1.20,413),rep(1.00,935)) #HR upper 95% CI
HR_UB<-c(
  rep(68,7),
  rep(13.6,21),rep(6.41,84),rep(1.90,413),rep(1.00,935)) #HR lower 95% CI
...
plot_graph(num_subgroups = 4,#life_table1 = life_table_venous_combined, 
           ...
           graph_name = 'Venous_event_graph_flu_pneumonia2',
           levels = c(#'Any influenza/pneumonia infection',
                      'Female','Male',
                      #                      'hospitalised influenza/pneumonia infection',
                      #                      'non-hospitalised influenza/pneumonia infection <60 years old',
                      '<60 years old',
                      #                      'non-hospitalised influenza/pneumonia infection 60+ years old',
                      '60+ years old'
           ),
...
print("Influenza/pneumonia excess events for Arterial outcome")


#Female
print("Female")

return_daily_incidence<-average_daily_incidence(dataset=data_female, disease_event_date=data_female$OUT_ARTERIAL_EVENT, covid_date=data_female$EXP_CONFIRMED_FLU_PNEUMONIA_DATE, futime=data_female$futime,
                                                max_follow_up_time=max_follow_up_time)

HR<-c(
  rep(16.2,7),
  rep(3.88,21),rep(1.92,84),rep(1.48,413),rep(1.00,935))
HR_LB<-c(
  rep(13.6,7),
  rep(3.11,21),rep(1.60,84),rep(1.32,413),rep(1.00,935)) #HR upper 95% CI
HR_UB<-c(
  rep(19.2,7),
  rep(4.85,21),rep(2.29,84),rep(1.66,413),rep(1.00,935)) #HR lower 95% CI
...
return_daily_incidence<-average_daily_incidence(dataset=data_male, disease_event_date=data_male$OUT_ARTERIAL_EVENT, covid_date=data_male$EXP_CONFIRMED_FLU_PNEUMONIA_DATE, futime=data_male$futime,
                                                max_follow_up_time=max_follow_up_time)

HR<-c(
  rep(16.6,7),
  rep(3.94,21),rep(2.03,84),rep(1.36,413),rep(1.00,935))
HR_LB<-c(
  rep(14.2,7),
  rep(3.21,21),rep(1.73,84),rep(1.21,413),rep(1.00,935)) #HR upper 95% CI
HR_UB<-c(
  rep(19.5,7),
  rep(4.83,21),rep(2.40,84),rep(1.53,413),rep(1.00,935)) #HR lower 95% CI
...
return_daily_incidence<-average_daily_incidence(dataset=data_young, disease_event_date=data_young$OUT_ARTERIAL_EVENT, covid_date=data_young$EXP_CONFIRMED_FLU_PNEUMONIA_DATE, futime=data_young$futime,
                                                max_follow_up_time=max_follow_up_time)

HR<-c(
  rep(19.3,7),
  rep(2.56,21),rep(1.22,84),rep(1.04,413),rep(1.00,935))
HR_LB<-c(
  rep(12.3,7),
  rep(1.22,21),rep(0.69,84),rep(0.75,413),rep(1.00,935)) #HR upper 95% CI
HR_UB<-c(
  rep(30,7),
  rep(5.37,21),rep(2.15,84),rep(1.44,413),rep(1.00,935)) #HR lower 95% CI
...
return_daily_incidence<-average_daily_incidence(dataset=data_old, disease_event_date=data_old$OUT_ARTERIAL_EVENT, covid_date=data_old$EXP_CONFIRMED_FLU_PNEUMONIA_DATE, futime=data_old$futime,
                                                max_follow_up_time=max_follow_up_time)

HR<-c(
  rep(16.2,7),
  rep(4.01,21),rep(2.05,84),rep(1.47,413),rep(1.00,935))
HR_LB<-c(
  rep(14.3,7),
  rep(3.44,21),rep(1.81,84),rep(1.35,413),rep(1.00,935)) #HR upper 95% CI
HR_UB<-c(
  rep(18.3,7),
  rep(4.68,21),rep(2.32,84),rep(1.60,413),rep(1.00,935)) #HR lower 95% CI
...
#4. Plot changes version 2
plot_graph(num_subgroups = 4,#life_table1 = life_table_arterial_combined, 
           ...
           graph_name = 'Arterial_event_graph_flu_pneumonia2',
           levels = c(#'Any influenza/pneumonia infection',
                      'Female','Male',
                      #                      'hospitalised influenza/pneumonia infection',
                      #                      'non-hospitalised influenza/pneumonia infection <60 years old',
...
#grid.arrange(Venous_event_graph_flu_pneumonia, Arterial_event_graph_flu_pneumonia, ncol=2)
Venous_event_graph_flu_pneumonia2<-Venous_event_graph_flu_pneumonia2 + theme(axis.title.y = element_blank())
Arterial_event_graph_flu_pneumonia2<-Arterial_event_graph_flu_pneumonia2 + theme(axis.title.y = element_blank())

FLU_plot<-ggarrange(Arterial_event_graph_flu_pneumonia2, Venous_event_graph_flu_pneumonia2, common.legend=TRUE, ncol=2, legend="bottom")
ggarrange(Arterial_event_graph_flu_pneumonia2, Venous_event_graph_flu_pneumonia2, common.legend=TRUE, ncol=2, legend="bottom")

dev.print(file = "P:/keenes/Projects/CCU002-04/results/FLU_cox_analysis/AER/AER_FLU_week1.png", device=png, width = 800, height = 400)
