--************************************************************************************************
-- Script:       2-5_PHEN_medications.sql
...
CREATE TABLE SAILWWMCCV.PHEN_WDDS_COVARIATES (
    ...
    oral_anticoagulants                        smallint,
    anticoagulant                              smallint, 
    lipid_regulating_drugs                     smallint,
    insulin                                    smallint,
...
INSERT INTO SAILWWMCCV.PHEN_WDDS_COVARIATES (alf_e,bnf_combined,dmdcode_prescribed,dt_prescribed,prac_cd_e)
    SELECT alf_e, bnf_combined, dmdcode_prescribed, dt_prescribed, prac_cd_e
    FROM SAILWMCCV.C19_COHORT20_RRDA_WDDS
    WHERE bnf_combined IN (SELECT code FROM SAILWWMCCV.PHEN_BNF_OTHER_DIABETIC
                           WHERE is_latest = 1)
     AND (alf_e IN (SELECT alf_e FROM SAILWMCCV.C19_COHORT20) OR alf_e IN (SELECT DISTINCT alf_e FROM SAILWMCCV.C19_COHORT16));
...
ALTER TABLE SAILWWMCCV.PHEN_WDDS_COVARIATES ADD c19_cohort16 SMALLINT NULL;
--------------------------------------------------------------------------------------------------
-- Update C20 flag
UPDATE SAILWWMCCV.PHEN_WDDS_COVARIATES SET c19_cohort20 = 1
WHERE alf_e IN (SELECT DISTINCT alf_e FROM SAILWMCCV.C19_COHORT20);

--------------------------------------------------------------------------------------------------
-- Update C16 flag
UPDATE SAILWWMCCV.PHEN_WDDS_COVARIATES SET c19_cohort16 = 1
