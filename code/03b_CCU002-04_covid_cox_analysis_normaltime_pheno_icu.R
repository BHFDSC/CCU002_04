...
if (vaccine=="vaccine_start") {
  df$cohort_start_date <- ifelse(is.na(df$COV_COVID_VACCINATION_FIRST_DOSE_DATE),as.Date("2020-01-01"), df$COV_COVID_VACCINATION_FIRST_DOSE_DATE)
  df$cohort_end_date <- as.Date("2021-12-31")
} else if (vaccine=="vaccine_end") {
  df$cohort_start_date <- as.Date("2020-01-01")
  df$cohort_end_date <- ifelse(is.na(df$COV_COVID_VACCINATION_FIRST_DOSE_DATE),as.Date("2021-12-31"), df$COV_COVID_VACCINATION_FIRST_DOSE_DATE)
} else {
  df$cohort_start_date <- as.Date("2020-01-01")
  df$cohort_end_date <- as.Date("2021-12-31")
}
...
#pheno_of_interest<-"hospitalised"
  ...
  data$EXP_CONFIRMED_COVID_PHENOTYPE <- ifelse(!is.na(data$ICU_OR_RESP_SUPPORT), data$ICU_OR_RESP_SUPPORT, data$EXP_CONFIRMED_COVID_PHENOTYPE)
  ...
  data$expo_date <- as.Date(ifelse((data$EXP_CONFIRMED_COVID_PHENOTYPE %in% pheno_of_interest),
                                   data$EXP_CONFIRMED_COVID19_DATE, NA), origin='1970-01-01') #phenotype: code added
...
#                                      ((data$overall_event_date <  data$pt_end_date) & (data$overall_event_date == data$DATE_EXPO_CENSOR)) ), 1, 0) #phenotype: code added
    ...
    # #ind_any_zeroeventperiod <- any((tbl_event_count$events_total == 0) & (!identical(cuts_weeks_since_expo, c(4, 49))))
     ...
     write.csv(tbl_event_count, paste0("P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/event_counts/tbl_event_count_COVID_icu_",outcome_list2,"_",time,"_",vaccine,"_",censor,"_",excl,"_",Sys.Date(),".csv"), row.names = T)
    ...
    # #=============================================================================
     ...
     incrate <- data.table::fread(paste0("P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/event_counts/tbl_event_count_COVID_icu_",outcome_list2,"_",time,"_",vaccine,"_",censor,"_",excl,"_",Sys.Date(),".csv"),
                                  data.table = FALSE)
     ...
     write.table(inc_rate3, paste0("P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/incidence/tbl_incidence_rate_COVID_icu_overall_",time,"_",vaccine,"_",censor,"_",excl,"_",Sys.Date(),".csv"), append=TRUE, col.name=!file.exists(paste0("P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/incidence/tbl_incidence_rate_COVID_icu_overall_",time,"_",vaccine,"_",censor,"_",excl,"_",Sys.Date(),".csv")))
     write.table(inc_rate4, paste0("P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/incidence/tbl_incidence_rate_COVID_icu_infected_",time,"_",vaccine,"_",censor,"_",excl,"_",Sys.Date(),".csv"), append=TRUE, col.name=!file.exists(paste0("P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/incidence/tbl_incidence_rate_COVID_icu_infected_",time,"_",vaccine,"_",censor,"_",excl,"_",Sys.Date(),".csv")))
     write.table(inc_rate5, paste0("P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/incidence/tbl_incidence_rate_COVID_icu_controls_",time,"_",vaccine,"_",censor,"_",excl,"_",Sys.Date(),".csv"), append=TRUE, col.name=!file.exists(paste0("P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/incidence/tbl_incidence_rate_COVID_icu_controls_",time,"_",vaccine,"_",censor,"_",excl,"_",Sys.Date(),".csv")))
    ...
    write(c("event term estimate conf.low conf.high std.error n.event subgroup"), 
          paste0('P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/',time,'/',adjust,'_cox_results_COVID_icu_',Sys.Date(),'_',vaccine,"_",outcome_list2,"_",censor,"_",excl,'.csv'), append = TRUE)
    ...
    purrr::map(outcome_list, function(x) {
      ...
      cat(paste0(substring(x,9,40),' ',cov, apply(s$coefficients, 1, 
                 ...
                 collapse = '\n'), '\n', sep = '', 
          file = paste0('P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/',time,'/',adjust,'_cox_results_COVID_icu_',Sys.Date(),'_',vaccine,"_",outcome_list2,"_",censor,"_",excl,'.csv'), append = TRUE)
      invisible(model)
    })
    
    #Read in "Extensive..." results files after running Coxph then do the following
    estimates <- data.table::fread(paste0('P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/',time,'/',adjust,'_cox_results_COVID_icu_',Sys.Date(),'_',vaccine,"_",outcome_list2,"_",censor,"_",excl,'.csv'),
                                   data.table = FALSE)
    ...
    #Then save to csv.
    write.table(estimates, paste0('P:/keenes/Projects/CCU002-04/results/COVID_cox_analysis/',time,'/',adjust,'_cox_results_COVID_icu_',Sys.Date(),'_',vaccine,"_",outcome_list2,"_",censor,"_",excl,'.csv'), row.names = FALSE)
